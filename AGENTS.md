...
- **Over capacity:** Only paid paths can oversell a class (`forceClaimSeat`), and they must: refusing someone already charged is the wrong outcome. Every breach is logged from the webhook and the class carries an "over capacity" flag in the occupancy column of `/admin/schedule`, so Gabrielle hears about it before the class rather than in the room.
